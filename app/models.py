...
class Cohorte(db.Model):
    ...
    id_cohorte = db.Column(db.Integer, nullable=False,
...
class Groupe(db.Model):
    ...
    id_groupe = db.Column(db.Integer, nullable=False,
                          autoincrement=True, primary_key=True)
    nom_groupe = db.Column(db.String(32))
    cohorte_id = db.Column(db.Integer, db.ForeignKey(
        "cohorte.id_cohorte"), nullable=False)
...
class Utilisateur(UserMixin, db.Model):
    ...
    niveaux_diff = db.relationship(
        "Difficulte", secondary='predictions_utilisateur_difficultes')
...
class Phrase(db.Model):
    ...
    id_phrase = db.Column(db.Integer, nullable=False,
    ...
    difficulte_id = db.Column(db.Integer, db.ForeignKey(
        "difficulte.id_difficulte"), nullable=False, index=True)
    ouvrage_id = db.Column(db.Integer, db.ForeignKey(
        "ouvrage.id_ouvrage"), nullable=False)
...
class Ouvrage(db.Model):
    ...
    id_ouvrage = db.Column(db.Integer, nullable=False,
                           autoincrement=True, primary_key=True)
...
class Difficulte(db.Model):
    ...
    id_difficulte = db.Column(
        db.Integer, nullable=False, autoincrement=True, primary_key=True, index=True)
    ...
    vers_utilisateurs = db.relationship(
        "Utilisateur", secondary='predictions_utilisateur_difficultes')
...
class Test(db.Model):
    ...
    def __repr__(self):
        return "<test('%s', '%s', '%s', '%s', '%s')>" % \
            (self.id_test, self.utilisateur_id,
             self.heure_debut, self.heure_fin, self.controle)


class Reponse(db.Model):
    __tablename__ = "reponse"
    id_reponse = db.Column(db.Integer, nullable=False,
                           autoincrement=True, primary_key=True)
    test_id = db.Column(db.Integer, db.ForeignKey("test.id_test"), index=True)
    phrase_id = db.Column(db.Integer, db.ForeignKey(
        "phrase.id_phrase"), index=True)
    ...
    def __repr__(self):
        return "<reponse('%s', '%s', '%s', '%s', '%s')>" % \
            (self.id_reponse, self.test_id, self.phrase_id,
             self.contenu_reponse, self.duree)
...
class TypeTest(db.Model):
    ...
    id_type_test = db.Column(db.Integer, nullable=False,
                             autoincrement=True, primary_key=True)
...
class StatsPhrases(db.Model):
    ...
    phrase_id = db.Column(db.Integer, db.ForeignKey(
        "phrase.id_phrase"), nullable=False, primary_key=True)
...
class StatsDifficultes(db.Model):
    ...
    difficulte_id = db.Column(db.Integer, db.ForeignKey(
        "difficulte.id_difficulte"), nullable=False, primary_key=True)
...
class StatsTests(db.Model):
    ...
    test_id = db.Column(db.Integer, db.ForeignKey(
        "test.id_test"), nullable=False, primary_key=True)
    # Cronbach's Alpha
    cr_alpha = db.Column(db.Float)
    # Standard error of mesurement
    sem = db.Column(db.Float)
...
class LienUtilisateursDifficultes(db.Model):
    ...
    difficulte_id = db.Column(db.Integer, db.ForeignKey(
        "difficulte.id_difficulte"), primary_key=True)
