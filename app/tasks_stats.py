...
def calcul_p_value():
    ...
    for row in db.session.query(Phrase):
        nb_rep_correctes = db.session.query(Phrase).join(Reponse).filter(Reponse.phrase_id==row.id_phrase).filter(Reponse.contenu_reponse==Phrase.element_reponse).count()
        nb_total_rep = db.session.query(Reponse).filter(Reponse.phrase_id==row.id_phrase).count()
        ...
        if nb_total_rep == 0:
            nb_total_rep = 1
        data = StatsPhrases(phrase_id=row.id_phrase, p_value = nb_rep_correctes / nb_total_rep, nb_reponses = nb_total_rep)
...
def calcul_centile():
	...
	lst = [x[0] for x in sorted(db.session.query(Stats_phrases.phrase_id, Stats_phrases.p_value).all(),key=lambda x: x[1], reverse=True)]
	for item in range(len(lst)):
		data = Stats_phrases(phrase_id=lst[item], centile_diff=int(round(item/len(lst), 2)*100))
		#merge permet de faire un create or update
		db.session.merge(data)
	db.session.commit()
...
def calcul_discrimination_index():
	...
	for utilisateur in [x for x, in db.session.query(Utilisateur.id).all()]:
		...
		for difficulte in [x for x, in db.session.query(Difficulte.id_difficulte).all()]:
			rep_concernees = reponses.join(Phrase).filter(Phrase.difficulte_id==difficulte)
			...
			score = rep_concernees.filter(Reponse.contenu_reponse==Phrase.element_reponse).count() / rep_concernees.count()
	...
	for difficulte in db.session.query(Difficulte).all():
		...
		lst_comp_loc = sorted([x for x in liste_competences if x[1] == difficulte], key=lambda x: x[2])
		
		#Si moins de 10 personne dans le groupe de détermination des compétences: on continue
		#if len(lst_comp_loc) < 10:
		#	continue
		
		bons = [x[0] for x in lst_comp_loc if lst_comp_loc.index(x)/len(lst_comp_loc) >= 0.73]
		mauvais = [x[0] for x in lst_comp_loc if lst_comp_loc.index(x)/len(lst_comp_loc) >= 0.27]
		...
		for phrase in db.session.query(Phrase).filter(Phrase.difficulte_id==difficulte.id_difficulte):
			...
			discrimination_index_phrase = (nb_bonnes_reponses_bons / nb_reponses_bons) - (nb_bonnes_reponses_mauvais / nb_reponses_mauvais)
			...
			if niv_diff == 1:
				data = Stats_phrases(phrase_id=phrase.id_phrase, discrimination_index_niv1=discrimination_index_phrase)
			elif niv_diff == 2:
				data = Stats_phrases(phrase_id=phrase.id_phrase, discrimination_index_niv2=discrimination_index_phrase)
			else:
				data = Stats_phrases(phrase_id=phrase.id_phrase, discrimination_index_niv3=discrimination_index_phrase)
