...
def questions():
    ...
    if 'questions_posées' not in session:
        ...
        session['question'] = next_question(session['questions_posées'], session['reponses_eval'])
    ...
    if session['question'] == None:
        ...
        db.session.query(Test).filter(Test.id_test == session['id_test']).update({'heure_fin': datetime.utcnow()})
        db.session.commit()
        return redirect(url_for('questionnaire.bilan'))        
    
    
    #Récupération des références de l'oeuvre
    data_oeuvre = Ouvrage.query.filter(Ouvrage.id_ouvrage==dic_question['ouvrage_id']).first()    
    ...
    if form.validate_on_submit():
        ...
        rep = Reponse(duree = session['durees_rep'][-1], test_id = session['id_test'], phrase_id = session['question'], contenu_reponse = contenu_reponse)
        db.session.add(rep)
        ...
        session['question'] = next_question(questions=session['questions_posées'], reponses=session['reponses_eval'])
...
def bilan():
    ...
    set_diff_2 = [(x[0], x[1], ' - '.join(x[1:]))  if x[2] != '' else (x[0], x[1]) for x in db.session.query(Difficulte.id_difficulte, Difficulte.type_difficulte, Difficulte.type_difficulte2).distinct().all()]
    ...
    for id_d, _, nom in set_diff_2:
        ...
        nb_bonnes_rep = len([x for x in zip(session["id_ difficultes"], session["reponses_eval"]) if x[0] == id_d and x[1] == 1])
    ...
    #Récup no cohorte
    no_cohorte = db.session.query(Cohorte.id_cohorte).join(Groupe).join(Utilisateur).filter(Groupe.id_groupe==current_user.groupe_id).first()[0]
    ...
    for item in set_diff_1:
        ...
        lst_diff_r1_cohorte.append(nb_bonnes_rep_cohorte / nb_tot_rep_cohorte *100)
